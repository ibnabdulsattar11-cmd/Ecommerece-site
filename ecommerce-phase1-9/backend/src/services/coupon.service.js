const { Coupon, CouponUsage, Product } = require("../models");
...
const resolveEligibleSubtotal = async (coupon, items) => {
  const [scopedProducts, scopedCategories] = await Promise.all([coupon.getProducts(), coupon.getCategories()]);
  ...
  for (const item of items) {
    ...
    const product = await Product.findByPk(item.productId, { attributes: ["id", "categoryId"] });
    if (product && categoryIds.has(product.categoryId)) eligible += item.lineTotal;
  }
  return eligible;
};
...
const validateCoupon = async (code, { userId, items }) => {
  ...
  if (new Date(coupon.expiryDate) < new Date()) throw new ApiError(400, "This coupon has expired");
  ...
  if (coupon.isUserSpecific) {
    ...
    if (eligibleUsers.length === 0) throw new ApiError(403, "This coupon is not available on your account");
  }
  ...
  if (eligibleSubtotal <= 0) {
    throw new ApiError(400, "This coupon does not apply to any items in your cart");
  }
  if (eligibleSubtotal < parseFloat(coupon.minOrder)) {
    throw new ApiError(400, `Minimum order of ${coupon.minOrder} required for this coupon`);
  }

  let discount =
    coupon.type === "percentage" ? (eligibleSubtotal * parseFloat(coupon.value)) / 100 : parseFloat(coupon.value);

  discount = Math.min(discount, eligibleSubtotal);
  if (coupon.maxDiscount !== null) discount = Math.min(discount, parseFloat(coupon.maxDiscount));

  return { coupon, discount: parseFloat(discount.toFixed(2)), eligibleSubtotal };
};
