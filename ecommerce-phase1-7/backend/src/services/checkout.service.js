...
const priceCartItems = async (cartItems) => {
  ...
  for (const item of cartItems) {
    ...
    if (!product || product.status !== "active") {
      throw new ApiError(400, `"${product?.nameEn || "A product"}" in your cart is no longer available`);
    }
    ...
    if (item.variantId) {
      ...
      if (!variant) throw new ApiError(400, `A selected option for "${product.nameEn}" is no longer available`);
    ...
    priced.push({
      ...
      variantSnapshot: variant ? { size: variant.size, color: variant.color } : null,
    });
  }
...
const resolveShippingAddress = async ({ user, addressId, shippingAddress }) => {
  if (addressId) {
    if (!user) throw new ApiError(401, "Log in to use a saved address, or provide a shipping address directly");
  ...
  if (shippingAddress) {
    const required = ["name", "phone", "email", "country", "city", "area", "street"];
    const missing = required.filter((f) => !shippingAddress[f]);
    if (missing.length) throw new ApiError(400, `Missing shipping details: ${missing.join(", ")}`);
  ...
  for (const item of items) {
    if (item.variantId) {
      const [count] = await ProductVariant.decrement(
        { stock: item.quantity },
        { where: { id: item.variantId, stock: { [Op.gte]: item.quantity } }, transaction }
      );
      if (!count) throw new ApiError(409, `"${item.productNameSnapshot}" just went out of stock`);
    } else {
      const [count] = await Product.decrement(
        { stock: item.quantity },
        { where: { id: item.productId, stock: { [Op.gte]: item.quantity } }, transaction }
      );
      if (!count) throw new ApiError(409, `"${item.productNameSnapshot}" just went out of stock`);
    }
  }
...
const restoreStock = async (items, transaction) => {
  for (const item of items) {
    if (item.variantId) {
      await ProductVariant.increment({ stock: item.quantity }, { where: { id: item.variantId }, transaction });
    } else {
      await Product.increment({ stock: item.quantity }, { where: { id: item.productId }, transaction });
    }
  }
};
 ...
 * happens next.
 */
const buildOrder = async ({ user, cartId, cartItems, addressId, shippingAddress, couponCode, paymentMethod }) => {
  const items = await priceCartItems(cartItems);
  const subtotal = parseFloat(items.reduce((sum, i) => sum + i.lineTotal, 0).toFixed(2));

  const { addressId: resolvedAddressId, snapshot } = await resolveShippingAddress({
    user,
    addressId,
    shippingAddress,
  });

  const delivery = await deliveryZoneService.checkDelivery({ city: snapshot.city, area: snapshot.area });
  if (!delivery.available) {
    throw new ApiError(400, delivery.reason || "We do not deliver to this location yet");
  }

  let discount = 0;
  let couponResult = null;
  if (couponCode) {
    couponResult = await couponService.validateCoupon(couponCode, { userId: user?.id, items });
  ...
  const total = parseFloat((subtotal - discount + shippingCharge + tax).toFixed(2));
  ...
  const order = await sequelize.transaction(async (t) => {
    ...
    if (couponResult) {
      ...
      await CouponUsage.create(
        { couponId: couponResult.coupon.id, userId: user?.id || null, orderId: newOrder.id, discountApplied: discount },
        { transaction: t }
      );
    }
 ...
 */
const cancelOrderAndRestoreStock = async (order, { reason = "Payment failed", status = "cancelled" } = {}) => {
